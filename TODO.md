...
10. 🔥 **СЛЕДУЮЩЕЕ:** Система отзывов и рейтингов (звёзды/баллы)
    - [ ] Backend: CRUD эндпоинты для отзывов на пресеты
    - [ ] Backend: Система рейтингов (1-5 звёзд)
    - [ ] Backend: Расчёт среднего рейтинга для пресетов
    - [ ] Frontend: UI для создания/редактирования отзывов
    - [ ] Frontend: Компонент рейтинга (звёзды)
    - [ ] Frontend: Отображение отзывов на странице пресета
    - [ ] Frontend: Отображение среднего рейтинга в карточках пресетов
    - [ ] Frontend: Фильтрация пресетов по рейтингу
11. 🔥 **СЛЕДУЮЩЕЕ:** Привязка принтеров к пресетам
    - [ ] Backend: Эндпоинты для управления связями пресет-принтер (добавить/удалить принтер к пресету)
    - [ ] Backend: Обновление схем PresetCreate/PresetUpdate для работы с printer_ids
    - [ ] Backend: Автоматическое создание связей при создании/обновлении пресета
    - [ ] Frontend: UI для выбора принтеров при создании/редактировании пресета (множественный выбор)
    - [ ] Frontend: Отображение списка принтеров на странице пресета
    - [ ] Frontend: Фильтрация пресетов по принтеру (уже частично работает, нужно доработать)
    - [ ] Frontend: Отображение иконок/названий принтеров в карточках пресетов
