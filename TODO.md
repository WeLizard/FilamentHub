...
- [ ] Изучить структуру `resources/profiles/` в OrcaSlicer — какие файлы, форматы, иерархия
- [ ] Изучить формат JSON профилей принтеров (machine): какие поля, зависимости, наследование
- [ ] Изучить формат JSON профилей филаментов (filament): связь с брендами, вариации по нозлам
- [ ] Изучить формат JSON профилей печати (process): связь с принтерами и материалами
- [ ] Изучить как OrcaSlicer загружает профили при запуске (код в `PresetBundle`, `Preset`, `VendorProfile`)
- [ ] Изучить формат vendor JSON (например `Elegoo.json`, `Creality.json`) — мета-описание всех профилей вендора
- [ ] Понять систему наследования профилей (`inherits`, `compatible_printers`, `compatible_prints`)
- [ ] Изучить процесс PR в OrcaSlicer для добавления профилей — что проверяют, какие требования
- [ ] Составить спецификацию: что нужно хранить на бэкенде FilamentHub для полной поддержки
- [ ] Определить MVP: минимальный набор для динамической загрузки без ломания совместимости
