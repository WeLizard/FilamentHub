...
- [x] ~~Admin endpoints не используются из frontend~~ `[CONTRACT-5]` — все admin endpoints реализованы в adminAPI (client.ts)
...
- [x] ~~Admin role check только на frontend~~ `[FRONTEND-1]` — все admin endpoints используют `Depends(get_current_admin_user)`
...
- [x] DeleteAccountModal — полная i18n (30+ строк)
- [x] Badge — labels/titles через i18n ключи
- [x] Dropdown — placeholder, emptyMessage, clearSelection
- [x] CreateFilamentModal — labels, placeholders, titles, options (50+ строк)
- [x] ColorMaterialSection — placeholder, title
- [x] CreatePresetModal — units, placeholders, defaults (~6 строк, остальное уже было)
- [x] CreatePrinterProfileModal — units, labels (~9 строк)
- [x] ProfilePage — labels, titles, actions
- [x] BrandProfilePage — regions, units, email (~5 строк)
- [x] AdminUsers — titles, confirmText
- [x] SettingsTab — полная i18n
- [x] AdminPrinters — units, labels, multimaterial (~12 строк)
- [x] AdminPrinterRequests — units (~1 строка)
- [x] ShareMenu — share options (~6 строк)
- [x] Notifications — formatTime, titles, buttons (~20 строк)
- [x] FeedbackModal — types, labels, buttons (~30 строк)
- [x] WikiFeedbackModal — labels, buttons (~13 строк)
- [x] MobileTocDrawer — titles (~4 строки)
- [x] ExportFromOrcaSlicerButton — states, errors (~6 строк)
- [x] ExportPrinterProfilesButton — states, tooltips (~8 строк)
- [x] ExportPrintProfilesButton — states, tooltips (~8 строк)
- [x] PresetSyncToggle — labels, errors (~6 строк)
- [x] CreatePrinterRequestModal — form labels, buttons (~25 строк)
- [x] CreateReviewModal — auth, form, buttons (~20 строк)
- [x] CreatePrintProfileModal — form, buttons (~20 строк)
- [x] ReviewCard — labels (~6 строк)
- [x] FilamentSummaryCard — noColor (~1 строка)
- [x] SEOHead — title, description
...
## CI/CD — OrcaSlicer форк

- [ ] Починить GitHub Actions в `lizardjazz1/OrcaSlicer` (branch `filamenthub-integration`)
  - "Build all" — падает (возможно secrets для подписи/нотаризации)
  - "Check profiles" — падает (кастомные профили FilamentHub)
  - "Check locale" — падает (новые строки без перевода)
  - Или отключить ненужные workflows если не планируем CI для форка
