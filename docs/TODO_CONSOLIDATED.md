...
Frontend Web UI      [██████████████████░░]  85%
OrcaSlicer C++       [██████████████████░░]  87%
Безопасность         [██████████████████░░]  90%  ← P0: 4 открыты (3 XSS + EXPORT-1)
Вендор-бандлы        [░░░░░░░░░░░░░░░░░░░░]   0%
Публичный запуск     [░░░░░░░░░░░░░░░░░░░░]   0%
```
...
- [x] ~~Blocking wait 30 сек в UI потоке~~ `[CRASH-1]` — не найдено в текущем коде (удалено/переработано)
...
- [ ] **get_my_presets не возвращает сохранённые чужие пресеты** — см. раздел «Фичи → Ближайшие»

### Закрытые P1

- [x] ~~m_is_syncing plain bool~~ `[RACE-1]` — переведён на `std::atomic<bool>`
- [x] ~~Blocking wait 30 сек~~ `[CRASH-1]` — не найден в текущем коде
...
- [ ] **Сохранённые чужие пресеты не попадают в OrcaSlicer** (~10-20 строк)
  - Кнопка 🔄 и toggle sync уже есть (`PresetSyncToggle.tsx`, `PATCH /saved-presets/{id}/sync`)
  - Поле `user_saved_presets.sync` в БД есть
  - **Осталось:** `GET /auth/my-presets` (`auth.py:395-468`) — дописать возврат сохранённых чужих пресетов с `sync=True`
  - Сейчас отдаёт только собственные пресеты, сохранённые из каталога игнорируются
