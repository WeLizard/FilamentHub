...
| [`plan_hh_integration.md`](plan_hh_integration.md) | **Happy Hare интеграция: preset-slot sync, HH snapshot, карта гейтов** |
...
- [ ] ~~Spoolman интеграция — `status="TODO"`~~ → заменено на Happy Hare интеграцию `[BACKEND-6]` → [`plan_hh_integration.md`](plan_hh_integration.md)
...
- [ ] **Happy Hare интеграция (preset-slot sync)** — [`plan_hh_integration.md`](plan_hh_integration.md) `[HH-INTEGRATION]`
  - [ ] Этап A: Backend модели (`UserPrinterDevice`, `PresetGateState`, `PresetUsageEvent`) + миграции
  - [ ] Этап B: Backend API (`preset-slots`, `devices`, `orca-preset-slot-sync`)
  - [ ] Этап C: Frontend MVP (карта слотов, назначение пресетов)
  - [ ] Этап D: Orca manual preset assignment (новые методы в `FilamentHubClient`)
  - [ ] Этап E: Orca HH snapshot sync (upload HH данных в FilamentHub)
  - [ ] Этап F: Optional spool layer (`UserSpool` CRUD, учёт остатков)

### Фичи (post-MVP)

#### Расширенный калькулятор 3D-печати (платная фича) `[CALC-PRO]`

> Источник: анализ `docs/3dcalc/` (PHP-калькулятор с G-code парсингом)

- [ ] **Парсинг G-code** — автоматическое извлечение параметров печати из файла
  - Поддержка слайсеров: OrcaSlicer, PrusaSlicer, Cura, Simplify3D, IdeaMaker, KISSlicer
  - Извлечение: время печати, вес материала, длина филамента, скорость, высота слоя
  - Извлечение 3D-превью (PNG thumbnail) из G-code
- [ ] **Профили принтеров** — мощность (Вт), стоимость принтера, амортизация, энергопотребление
  - Интеграция с существующей БД принтеров FilamentHub
- [ ] **Авто-подстановка материала** — fuzzy matching из каталога FilamentHub
  - По имени филамента из G-code → цена, плотность, вес катушки из БД
- [ ] **Расчёт стоимости из G-code** — загрузил файл → получил полную разбивку:
  - Стоимость материала, электроэнергия, амортизация принтера, наценка, итого
- [ ] **Админ-панель настроек калькулятора** — тарифы электроэнергии, валюта, наценки по умолчанию
- [ ] Монетизация: доступ по подписке или разовая покупка
...
- [ ] Гибридный sync OrcaSlicer release: GitHub Releases -> `backend/distributions/orcaslicer` (локальная папка как source of truth) `[INFRA-21]`
