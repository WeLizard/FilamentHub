...
def test_registry_mismatch_is_reported_when_field_inventory_changes(
    tmp_path: Path, capsys: pytest.CaptureFixture[str], monkeypatch: pytest.MonkeyPatch
) -> None:
    """Ordinary profile bytes do not version the schema; field shape does."""
    registry = tmp_path / "orca_field_registry.py"
    registry.write_text(
        'ORCA_FIELD_REGISTRY_VERSION = (\n    "field-inventory-sha256:'
        + "0" * 64
        + '"\n)\n',
        encoding="utf-8",
    )
    ...
    assert (
        refresh._check_registry_alignment(
            {"field_inventory_sha256": "1" * 64}, required=False
        )
        is False
    )
    ...
    assert "passthrough-контракту" in out


def test_registry_in_step_with_field_inventory_stays_quiet(
    tmp_path: Path, capsys: pytest.CaptureFixture[str], monkeypatch: pytest.MonkeyPatch
) -> None:
    digest = "2" * 64
    registry = tmp_path / "orca_field_registry.py"
    registry.write_text(
        f'ORCA_FIELD_REGISTRY_VERSION = (\n    "field-inventory-sha256:{digest}"\n)\n',
        encoding="utf-8",
    )
    monkeypatch.setattr(refresh, "FIELD_REGISTRY", registry)
    ...
    assert (
        refresh._check_registry_alignment(
            {"field_inventory_sha256": digest}, required=True
        )
        is True
    )
