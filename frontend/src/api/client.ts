...
export const notificationsAPI = {
  ...
  listFeed: async (params?: { limit?: number; cursor?: number; unread_only?: boolean }, signal?: AbortSignal): Promise<NotificationFeedResponse> => {
    const response = await api.get('/notifications/feed', { params, signal });
    return response.data;
  },

  get: async (notificationId: number, signal?: AbortSignal): Promise<Notification> => {
    const response = await api.get(`/notifications/${notificationId}`, { signal });
    return response.data;
  },
...
export const orcaslicerDeletedPresetsAPI = {
  list: async (
    ...
    signal?: AbortSignal,
  ): Promise<DeletedPresetDecisionFeedResponse> => {
    const response = await api.get(`/notifications/${notificationId}/deleted-presets`, { params, signal });
    return response.data;
  },
...
export const orcaSlicesAPI = {
  report: async (slices: OrcaSliceReportInput[]): Promise<OrcaSliceReportAccepted> => {
    const response = await api.post<OrcaSliceReportAccepted>('/orcaslicer/slices', { slices });
    return response.data;
  },
